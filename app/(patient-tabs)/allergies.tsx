    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(
            ...
            (snapshot) => {
                ...
                const allergyRecords = allRecords.filter(
                    (r: any) =>
                        r.tags?.includes('allergy') ||
                        r.type === 'allergy' ||
                        r.title?.toLowerCase().includes('allergy')
                );

                setRecords(allergyRecords);
                setLoading(false);
            },
            (error) => {
                ...
                {records.length > 0 ? (
                    records.map((record) => (
                        <TouchableOpacity
                            ...
                            onPress={() => {
                                // Navigate to main records page with filter/search? 
                                // Or just show details? For now, simpler to just view in main records if needed, 
                                // but user wants specific page. I'll make it a simple list card.
                            }}
                            activeOpacity={0.9}
                        >
                            <View style={styles.recordCardContent}>
                                <View style={styles.recordMain}>
                                    <View style={styles.recordLeft}>
                                        <View style={[styles.recordIcon, { backgroundColor: '#FEF9C3' }]}>
                    ...
                    <View style={styles.emptyState}>
                        <View style={[styles.emptyStateIconContainer, { backgroundColor: '#FEF9C3' }]}>
                            <AlertCircle size={48} color="#EAB308" />
                        </View>
                        <Text style={styles.emptyStateTitle}>No allergies recorded</Text>
                        <Text style={styles.emptyStateText}>
                            You haven't added any allergy records yet.
                        </Text>
                    </View>
                )}
            </ScrollView>
