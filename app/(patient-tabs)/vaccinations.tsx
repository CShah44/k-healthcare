    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(
            ...
            (snapshot) => {
                ...
                const vaccinationRecords = allRecords.filter(
                    (r: any) =>
                        r.type === 'vaccination' ||
                        r.tags?.includes('vaccination') ||
                        r.title?.toLowerCase().includes('vaccine') ||
                        r.title?.toLowerCase().includes('vaccination')
                );

                setRecords(vaccinationRecords);
                setLoading(false);
            },
            (error) => {
            ...
            >
                {records.length > 0 ? (
                            ...
                            <View style={styles.recordCardContent}>
                                <View style={styles.recordMain}>
                                    <View style={styles.recordLeft}>
                                        <View style={[styles.recordIcon, { backgroundColor: '#D1FAE5' }]}>
                    ...
                    <View style={styles.emptyState}>
                        <View style={[styles.emptyStateIconContainer, { backgroundColor: '#D1FAE5' }]}>
                            <Syringe size={48} color="#10B981" />
                        </View>
                        <Text style={styles.emptyStateTitle}>No vaccinations recorded</Text>
