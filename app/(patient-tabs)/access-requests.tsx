...
export default function AccessRequestsScreen() {
        ...
        snapshot.forEach((doc) => {
          ...
          const expiresAt =
            data.expiresAt instanceof Timestamp
              ...
              : new Date(data.expiresAt); // Handle Timestamp or Date string
          ...
          if (expiresAt > now) {
            fetchedRequests.push({
              ...
              avatar:
                data.doctorAvatar ||
                'https://images.pexels.com/photos/4173251/pexels-photo-4173251.jpeg?auto=compress&cs=tinysrgb&w=200', // Fallback
              expiresAt: expiresAt,
        ...
        snapshot.forEach((doc) => {
          ...
          if (expiresAt > now) {
            fetchedAccess.push({
              ...
              accessGrantedAt:
                data.accessGrantedAt instanceof Timestamp
                  ? data.accessGrantedAt.toDate()
                  : new Date(),
              expiresAt: expiresAt,
              active: data.active,
            });
  ...
  const confirmApproval = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Access Granted',
        `You have granted access to ${selectedRequest.doctorName} for ${selectedDuration} days.`,
      );
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              setRevoking(true);
              await updateDoc(doc(db, 'doctorAccess', accessId), {
                active: false,
              });
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                'Request Rejected',
                `You have rejected the request from ${name}.`,
              );
              // State updates automatically via onSnapshot
            } catch (error) {
        ...
        {activeTab === 'pending' ? (
          <>
            <View style={styles.infoBox}>
              ...
              <Text style={[styles.infoText, { color: colors.textSecondary }]}>
                Doctors listed here are requesting access to your medical
                records. Granting access will allow them to view your history
                and add new records.
              </Text>
            </View>
                ...
                <View key={request.id} style={styles.requestCard}>
                  ...
                  <View style={styles.actions}>
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        handleReject(request.id, request.doctorName)
                      }
                    >
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        handleApprove(request.id, request.doctorName)
                      }
                    >
          ...
          <>
            <View style={styles.infoBox}>
              ...
              />
              <Text style={[styles.infoText, { color: colors.textSecondary }]}>
                These doctors currently have access to your medical records. You
                can revoke their access at any time.
              </Text>
            </View>

            {activeAccessList.length > 0 ? (
              activeAccessList.map((access) => (
                <View key={access.id} style={styles.requestCard}>
                  <View style={styles.cardHeader}>
                    <Image
                  ...
                  <TouchableOpacity
                    style={[styles.actionButton, styles.rejectButton]}
                    onPress={() =>
                      handleRevokeAccess(access.id, access.doctorName)
                    }
                  >
                    <X size={18} color={Colors.medical.red} />
      ...
      >
        ...
        >
          <View style={styles.modalContent}>
            ...
            >
              How long should {selectedRequest?.doctorName} have access to your
              records?
            </Text>
                      ...
                      styles.optionLabel,
                      {
                        color:
                          selectedDuration === option.value
                            ...
                            : colors.text,
                        fontWeight:
                          selectedDuration === option.value ? '700' : '400',
                      },
                    ]}
                  >
                    {option.label}
